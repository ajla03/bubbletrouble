...
bool ResourceManager::LoadAll(HINSTANCE hInstance) {
    ...
    soundOn = LoadBitmap(hInstance, MAKEINTRESOURCE(IDB_SOUNDON));
    ...
    settingsMusicOnHover = LoadBitmap(hInstance, MAKEINTRESOURCE(IDB_MUSICON_HOVER));
    settingsSoundOff = LoadBitmap(hInstance, MAKEINTRESOURCE(IDB_SOUNDOFF));
    musicOff = LoadBitmap(hInstance, MAKEINTRESOURCE(IDB_MUSIC_OFF));
    musicOffHover =  LoadBitmap(hInstance, MAKEINTRESOURCE(IDB_MUSIC_OFF_HOVER));
    ...
    hero2Mask = LoadBitmap(hInstance, MAKEINTRESOURCE(IDB_HERO2_MASK));
    ...
    if (!character || !background || !heart || !restartButton) {
        MessageBox(NULL, "Ne mogu da u�itam sve resurse!", "Gre�ka", MB_ICONERROR);
        return false;
    }
